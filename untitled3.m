% 读取数据文件
data_file_path = '/dada/KIRU_ig1.xyz';
ref_file_path = '/dada/ITRF2008_GNSS.SSC.txt';
...
lambda = 0.01; % 岭回归参数
beta_north = (A' * A + lambda * eye(size(A, 2))) \ (A' * local_coords(:, 2)); % 拟合北向（N）

% 最小二乘法拟合，向东（E）
beta_east = (A' * A + lambda * eye(size(A, 2))) \ (A' * local_coords(:, 1)); % 拟合东向（E）

% 最小二乘法拟合，高度（H）
beta_height = (A' * A + lambda * eye(size(A, 2))) \ (A' * local_coords(:, 3)); % 拟合高度（H）
...
% 计算残差
residuals_north = local_coords(:, 2) - linear_trend_north;
residuals_east = local_coords(:, 1) - linear_trend_east;
residuals_height = local_coords(:, 3) - linear_trend_height;
